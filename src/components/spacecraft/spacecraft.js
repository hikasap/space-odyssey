...
export class Spacecraft {
    ...
    loadModel() {
        this.loader.load(
            ...
            (gltf) => {
                ...
                PhysicsInstance.addRigidBody(this.model);
    ...
    update(deltaTime) {
        ...
        for (let body of this.celestialBodiesInPullRadius) {
            // Apply gravitational force of unit magnitude towards the celestial body
            const direction = body.mesh.position.clone().sub(this.model.position);
            direction.normalize();
            const proximity = this.model.position.distanceTo(body.mesh.position);
            // The planet should pull stronger the closer the spacecraft is and also pull more if the planet is heavier
            const maxPull = 20;
            const pull = body.mass / (proximity * proximity * proximity);
            const pullStrength = Math.min(pull, maxPull);
            console.log('Pull Strength from planet:', pullStrength , body.celestialDetails['Name']);
            direction.multiplyScalar(pullStrength);
